...
export class LandinpageComponent implements OnInit {
  ...
  private baseUrl = `http://localhost/deskera/index.php`;

  constructor(private httpClient: HttpClient, private toastr: ToastrService) { }

  ngOnInit() {
    this.getCurrentCountry().subscribe((location) => {
  ...
  onSelect(country) {
    ...
    this.currentTime = formatDate(this.today, 'dd-MM-yyyy hh:mm:ss a', 'en-US', '+0530');;
  }

  saveData = (formValue) => {
    let saveform: Isaveform = {
    ...
    this.httpClient.post<void>(this.baseUrl + '/lead/savelead', saveform).subscribe(response => {
